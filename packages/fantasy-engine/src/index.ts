...
export function calculatePlayerScore(
  ...
  if (stats.penaltiesSaved > 0) {
    ...
    breakdown.push({ key: 'PENALTY_SAVED', points: pts, count: stats.penaltiesSaved });
    totalPoints += pts;
  }
  if (stats.penaltiesMissed > 0) {
    const pts = stats.penaltiesMissed * -2;
    breakdown.push({ key: 'PENALTY_MISSED', points: pts, count: stats.penaltiesMissed });
    totalPoints += pts;
  }
  ...
  if (stats.yellowCards > 0) {
    ...
    breakdown.push({ key: 'YELLOW_CARD', points: pts, count: stats.yellowCards });
    totalPoints += pts;
  }
  if (stats.redCards > 0) {
